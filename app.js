...
function render(){
  ...
  if(!visible.length){const isFiltering=filter!=='all'||locationFilter!=='all'||search;$('#empty-title').textContent=isFiltering?'조건에 맞는 음식이 없어요':'냉장고가 비어 있어요';$('#empty-copy').textContent=isFiltering?'다른 보관 장소나 검색 조건을 선택해 보세요.':'첫 음식을 등록하면 소비기한과 메뉴를 챙겨드릴게요.';emptyState.querySelector('[data-open-add]').hidden=Boolean(isFiltering)}
  updateSummary();renderMenus();renderHistory();
}
function updateSummary(){let urgent=0,safe=0;foods.forEach(item=>{const key=statusFor(daysLeft(item.expiry)).key;if(key==='urgent'||key==='expired')urgent++;else safe++});const locationCounts={fridge:0,freezer:0,room:0};foods.forEach(item=>locationCounts[item.location]++);const menus=possibleMenus();$('#total-count').textContent=foods.length;$('#mobile-total-count').textContent=foods.length;$('#urgent-count').textContent=urgent;$('#safe-count').textContent=safe;$('#recipe-count').textContent=menus.length;$('#location-all-count').textContent=foods.length;$('#location-fridge-count').textContent=locationCounts.fridge;$('#location-freezer-count').textContent=locationCounts.freezer;$('#location-room-count').textContent=locationCounts.room;const urgentNames=[...foods].sort((a,b)=>daysLeft(a.expiry)-daysLeft(b.expiry)).filter(item=>daysLeft(item.expiry)<=2).slice(0,2).map(item=>item.name);$('#mobile-overview-copy').textContent=urgentNames.length?`${urgentNames.join(' · ')} 먼저 확인해 보세요.`:foods.length?'아직 소비기한이 여유로워요.':'첫 음식을 등록해 보세요.'}
...
function updateStorageSuggestion(){
  ...
  if(!options){$('#expiry-date').value='';$('#suggestion-title').textContent='추천 정보가 없어요';$('#suggestion-copy').textContent='7일 · 14일 · 한 달 · 3개월 중 빠르게 선택하세요';$('#toggle-food-details').textContent='선택';$('#toggle-food-details').setAttribute('aria-expanded','false');$('#food-details').hidden=true;setSubmitReady(false);return}
